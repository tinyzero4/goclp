...
export image = `aws lightsail get-container-images --service-name canvas | jq -r '.containerImages[].image' | grep app | head -n 1`
export prometheus_image = `aws lightsail get-container-images --service-name canvas | jq -r '.containerImages[].image' | grep prometheus | head -n 1`
